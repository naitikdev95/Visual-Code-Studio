...
function pythonCompletions(context: CompletionContext): CompletionResult | null {
  ...
  const options = PYTHON_COMPLETIONS
    ...
    .map((kw) => ({
      label: kw,
      type: ["False","None","True","and","as","assert","async","await","break","class","continue","def","del","elif","else","except","finally","for","from","global","if","import","in","is","lambda","nonlocal","not","or","pass","raise","return","try","while","with","yield"].includes(kw)
        ? "keyword"
        : "__init__,__str__,__repr__".includes(kw) ? "method" : "function",
    }));
...
export function CodeEditor({ value, onChange, onRun, theme }: CodeEditorProps) {
  const extensions = [
    python(),
